...
describe('Fee feature Test', () => {
    ...
    it('When user click on fee link user should navigate to fee page', () => {
        ...
        cy.get('a[href="/fees/manage"]').should('exist')
        cy.get('a[href="/fees/add"]').should('exist')
        cy.contains('Transact').should('be.visible')
        cy.get('a[href="/fees/transactions"]').should('exist')
        cy.get('a[href="/fees/transactions/add"]').should('exist')
        cy.get('a:contains(Fees Structure )').should('exist')
        cy.get('a:contains(Fees Structure Coursewise )').should('exist')
        cy.get('a:contains(Fees Expected )').should('exist')
        cy.get('a:contains(Defaulters List )').should('exist')
        cy.get('a:contains(Adjustment Balance )').should('exist')
        cy.get('a:contains(Adjustment Transaction )').should('exist')
        cy.get('a:contains(Advance Balance )').should('exist')
        cy.get('a:contains(Fee Collection Datewise / Monthwise )').should('exist')
        cy.get('a:contains(Fees Paid After Academic Year )').should('exist')
        cy.get('a:contains(Fees Paid By TC Students )').should('exist')
        cy.get('a:contains(Fees Paid List )').should('exist')
        cy.get('a:contains(Receipt Transaction )').should('exist')
        cy.get('a:contains(Student Transaction )').should('exist')
        cy.get('a:contains(Transaction History )').should('exist')
    })
    ...
    it('When user click on disable link, selected transaction should get disabled', () => {
        ...
        cy.get('thead[class="bg-info"]').next().find('tr').last().find('a[class="text-danger"]').should('exist')
    ...
    it('when user enter all valid details in all textfield in fee add page and click on save button , fee details should get added and navigate to fee amnage', () => {
        cy.visit('/fees')
        cy.get('a[href="/fees/add"]').click()
        ...
        cy.get('span:contains(I)').siblings('input[type="checkbox"]').check({ multiple: true })
        cy.get('strong:contains(Payment Schedule)').click()
    ...
    it('When user click on fee name, user should navigate to fees/edit page', () => {
        ...
        // cy.contains('tr', 'Computer fee').within(() => { cy.contains('button', 'Computer fee Std III').click() })
        cy.contains('button', 'Computer fee Std III').click({force:true})
        cy.get('input[placeholder="Name"]').should('exist')
        cy.get('input[placeholder="Display Text"]').should('exist')
        cy.get('select[name="collect"]').should('exist')
        cy.get('input[placeholder="Tags"]').should('exist')
    ...
    it('When user click Receipt number, should navigate to transactions/edit page', () => {
        ...
        cy.get('i[class="fa fa-search"]').click()
        cy.get('tr').contains(Cypress.env('Recepiet')).click()
    ...
    it('When user click on Fees paid tab ,paid fees reciept details should display', () => {
        ...
        cy.get('input[type="search"]').type(Cypress.env('studentName')).next().children('ul').click()
        cy.wait(2000)
        cy.get('span:contains(Fees Paid)').click({ force: true })
    ...
    it('when user click on Defaulters List link, should navigate to Defaulters List Report page and able to download report ', () => {
        ...
        cy.get('span:contains(Course : )').should('be.visible').next('select').should('exist').select('I')
        cy.get('label:contains(Admission fee May 2023)').prev('input[type="checkbox"]').check({ force: true })
        cy.get('strong:contains(Customize)').parent().next().children().children('button[title="Download As PDF"]').should('exist').should('be.visible').next('button[title="Print"]').should('exist').should('be.visible').should('be.enabled')
        cy.get('button[title="Download As PDF"]').should('exist')
        cy.get('i[class="fa fa-print"]').click()
        cy.click_Back_Button() 
    ...
    it('Verify back button in Fees feature', () => {
        ...
        cy.click_Back_Button() 
        cy.url().should('eq', Cypress.config().baseUrl + '/fees/manage') 
        cy.get('a:contains(Fee)').parents('thead').next('tbody').children('tr').first().next().find('button').click({force: true})
      ...
      it('Verify back button in Transact feature', () => {
        ...
        cy.click_Back_Button() 
        cy.url().should('eq', Cypress.config().baseUrl + '/fees/transactions') 
        cy.get('a:contains(Receipt No)').parents('thead').next('tbody').children('tr').first().find('a').first().click()
        cy.url().should('eq', Cypress.config().baseUrl + '/fees/transactions/edit') 
        cy.click_Back_Button() 
        cy.url().should('eq', Cypress.config().baseUrl + '/fees/transactions')
        cy.click_Back_Button() 
        cy.url().should('eq', Cypress.config().baseUrl + '/fees') 
