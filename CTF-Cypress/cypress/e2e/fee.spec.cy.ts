...
describe('Fee feature Test', () => {
    ...
    it('When user click on fee link user should navigate to fee page', () => {
        ...
        cy.get('li.list-group-item').contains('Fees').within(() => {
          cy.get('a[ng-reflect-router-link="/fees/manage"]').should('have.attr', 'href', '/fees/manage')
          cy.get('a[ng-reflect-router-link="/fees/add"]').should('have.attr', 'href', '/fees/add')
          cy.contains('Search/Edit Fee').should('be.visible')
          cy.contains('Add fee').should('be.visible')
        })
        cy.contains('Transact').should('be.visible');
        cy.get('li.list-group-item').contains('Transact').within(() => {
          cy.get('a[ng-reflect-router-link="/fees/transactions"]').should('have.attr', 'href', '/fees/transactions')
          cy.get('a[ng-reflect-router-link="/fees/transactions/add"]').should('have.attr', 'href', '/fees/transactions/add')
          cy.contains('Search/View transaction').should('be.visible')
          cy.contains('New transaction').should('be.visible')
        })
        cy.contains('Fee Pending Table').should('be.visible')
        cy.contains('Fee Paid ').should('be.visible')
        cy.contains('Yearwise Fees Collection').should('be.visible')
        cy.contains('Institute-wise Fees Collection and Defaulters').should('be.visible')
        cy.contains('Institute-wise Fees Collection').should('be.visible')
        cy.contains('Fees Setup').should('be.visible')
        cy.contains('Fees Exception Reports').should('be.visible')
        cy.contains('Fees Transaction').should('be.visible')
   
    })
    it('When user click on fees add link user should navigate to fees/add page', () => {
        cy.visit('/fees')
        cy.get('a[href="/fees/add"]').click()
    ...
    it('when user click on Transaction History link, should navigate to Transaction History Report page and able to download report ', () => {
        cy.visit('/fees')
        cy.get('a[href="/fees/transactionHistoryReport"]').click()
        cy.url().should('contain', '/fees/transactionHistoryReport')
        cy.get('i[class="fa fa-print"]').click().screenshot()
    })
    it('when user click on Receipt Transaction link, should navigate to fees Receipt Transaction Report page and able to download report ', () => {
        cy.visit('/fees')
        cy.get('a[href="/fees/feesReceiptTransactionReport"]').click()
        cy.url().should('contain', '/fees/feesReceiptTransactionReport')
        cy.get('h3').should('contain.text', 'Fees Receipt Transaction Report')
        //cy.get('span:contains(From Date :)').next().children('input[name="reportFromDate"]').type(Cypress.env('DateFrom')).parent().siblings('span:contains(To Date :)').next().children('input[name="reportToDate"]').should('exist')
        cy.get('i[class="fa fa-print"]').should('exist')
    })
    it('when user click on Fees Paid List link, should navigate to fees Paid Report page and able to download report ', () => {
        cy.visit('/fees')
        cy.get('a[href="/fees/feesPaidReport"]').click()
        cy.url().should('contain', '/fees/feesPaidReport')
        cy.get('h3').should('contain.text', 'Fees Fees Paid List Report')
        //cy.get('span:contains(From Date :)').next().children('input[name="reportFromDate"]').type(Cypress.env('DateFrom')).parent().siblings('span:contains(To Date :)').next().children('input[name="reportToDate"]').should('exist')
        cy.get('span:contains(Course : )').next('select').select('PUC I PCME')
        cy.get('label:contains(Admission fee Annual)').prev('input[type="checkbox"]').check()
        cy.get('i[class="fa fa-print"]').should('exist')
    })
    it('when user click on Fees Structure Coursewise link, should navigate to Coursewise Fees Structure Report page and able to download report ', () => {
        cy.visit('/fees')
        cy.get('a[href="/fees/coursewiseFeesStructureReport"]').click()
        cy.url().should('contain', '/fees/coursewiseFeesStructureRepor')
    ...
    it('when user click on Fees Structure link, should navigate to Fees Structure Report page and able to download report ', () => {
        cy.visit('/fees')
        cy.get('a[href="/fees/feesStructureReport"]').click()
    ...
    it('when user click on Fees Fees Expected link, should navigate to Fees Expected Report page and able to download report ', () => {
        cy.visit('/fees')
        cy.get('a[href="/fees/feesExpectedReport"]').click()
        ...
        cy.get('span:contains(--- Select Fees ---)').next().find('span[class="dropdown-down"]').click({ force: true })
        cy.get('div:contains(Select All)').prev('input[type="checkbox"]').check({ force: true })
        cy.get('strong:contains(Customize)').parent().next().children().children('button[title="Download As PDF"]').should('exist').should('be.visible').next('button[title="Print"]').should('exist').should('be.visible').should('be.enabled')
        cy.get('button[title="Download As PDF"]').should('exist')
        cy.get('i[class="fa fa-print"]').click()
    })
    it('when user click on Defaulters List link, should navigate to Defaulters List Report page and able to download report ', () => {
        cy.visit('/fees')
        cy.get('a[href="/fees/feesDefaultersListReport"]').click()
        cy.url().should('contain', '/feesDefaultersListReport')
        cy.get('h3').should('contain.text', 'Fees Defaulters List Report')
        cy.get('span:contains(Course : )').should('be.visible').next('select').should('exist').select('PUC I PCME')
        cy.get('label:contains(Admission fee Annual)').prev('input[type="checkbox"]').check({ force: true })
        cy.get('strong:contains(Customize)').parent().next().children().children('button[title="Download As PDF"]').should('exist').should('be.visible').next('button[title="Print"]').should('exist').should('be.visible').should('be.enabled')
        cy.get('button[title="Download As PDF"]').should('exist')
        cy.get('i[class="fa fa-print"]').click()
    })
    it('when user click on Adjustment Transaction link, should navigate to Adjustment Transaction Report page and able to download report ', () => {
        cy.visit('/fees')
        cy.get('a[href="/fees/adjustmentTransactionReport"]').click()
        cy.wait(2000)
    ...
    it('when user click on Fee Collection Datewise / Monthwise link, should navigate to Fee Collection Datewise / Monthwise page and able to download report ', () => {
        cy.visit('/fees')
        cy.get('a[href="/fees/feesCollectedReport"]').click()
        cy.wait(2000)
    ...
    it('when user click on Fees Paid After Academic Year link, should navigate to Fees Paid After Academic Year page and able to download report ', () => {
        cy.visit('/fees')
        cy.get('a[href="/fees/feesPaidAfterAcadYearReport"]').click()
        cy.wait(2000)
    ...
    it('when user click on Fees Paid By TC Students link, should navigate to Fees Paid By TC Students report page and able to download report ', () => {
        cy.visit('/fees')
        cy.get('a[href="/fees/feesPaidByTCStudentsReport"]').click()
        cy.wait(2000)
    ...
    it('Verify back button in Fees feature', () => {
        cy.visit('/fees')
        cy.get('a[href="/fees/add"]').click({force: true})    
        cy.url().should('eq', Cypress.config().baseUrl + '/fees/add')  
        cy.click_Back_Button()   
        cy.url().should('eq', Cypress.config().baseUrl + '/fees')  
        cy.get('a[href="/fees/manage"]').click({force: true})  
        cy.url().should('eq', Cypress.config().baseUrl + '/fees/manage') 
        cy.get('button:contains(Add Fee)').click({force: true})                          
        cy.url().should('eq', Cypress.config().baseUrl + '/fees/add')  
        cy.click_Back_Button()    
        cy.wait(1500)
        cy.url().should('eq', Cypress.config().baseUrl + '/fees/manage') 
        cy.contains('a', /^Fee$/).parents('thead').next('tbody').children('tr').first().find('button:contains(Admission fee)').click({force: true}) 
        cy.url().should('eq', Cypress.config().baseUrl + '/fees/edit') 
        cy.click_Back_Button()   
        cy.url().should('eq', Cypress.config().baseUrl + '/fees/manage')
        cy.click_Back_Button()  
        cy.url().should('eq', Cypress.config().baseUrl + '/fees')                       
      ...
      it('Verify back button in Transact feature', () => {
        cy.visit('/fees')
        cy.get('a[href="/fees/transactions/add"]').click({force: true})    
        cy.url().should('eq', Cypress.config().baseUrl + '/fees/transactions/add')  
        cy.click_Back_Button()   
        cy.wait(2000)
        cy.url().should('eq', Cypress.config().baseUrl + '/fees') 
        cy.wait(1500)
        cy.get('a[href="/fees/transactions"]').click({force: true})  
        cy.wait(2500)
        cy.url().should('eq', Cypress.config().baseUrl + '/fees/transactions') 
        cy.get('a:contains(New Transaction)').click({force: true})                          
        cy.url().should('eq', Cypress.config().baseUrl + '/fees/transactions/add')  
        cy.click_Back_Button()    
        cy.url().should('eq', Cypress.config().baseUrl + '/fees/transactions') 
        cy.get('a:contains(Receipt No)').parents('thead').next('tbody').children('tr').first().children().next().next().find('a').first().click({force: true}) 
        cy.click_Back_Button() 
        cy.url().should('eq', Cypress.config().baseUrl + '/fees/transactions') 
        cy.click_Back_Button() 
        cy.url().should('eq', Cypress.config().baseUrl + '/fees')                       
