describe('Admission feature Test', () => {
  ...
  it('should navigate to admissions page successfully ', () => {
    ...
    cy.get('a[href="/admissions/seatMatrix/add"]').should('exist')
    cy.get('a[href="/admissions/seatMatrix/manage"]').should('exist')
    cy.get('a[href="/admissions/applications/manage"]').should('exist')
    cy.get('a[href="/admissions/shortlists/manage"]').should('exist')
    cy.contains('Course-wise Seat Allotment status ').should('be.visible')
    cy.contains('Caste-category breakup for the Year 2022').should('be.visible')
  })
  it(' should navigate to seatmatrix add page successfully', () => {
    cy.visit('/admissions')
    cy.get('a[href="/admissions/seatMatrix/add"]').click()
  ...
  it('When user select course from Course / Degree dropdown ,selected course should show ', () => {
    ...
    cy.contains('Course / Degree').should('exist')
    cy.get('select[name="course"]').should('exist')
    cy.get('select').select('B.Com 1').should('have.value', 'B.Com 1') // select option based on value
  })
  ...
  it('When click on applications should navigate to applications/manage page successfully ', () => {
    cy.visit('/admissions')
    cy.get('a[href="/admissions/applications/manage"]').click()
  ...
  it(' should navigate to seatmatrix add page successfully', () => {
    ...
    cy.get('a[href="/admissions/seatMatrix/add"]').click()
    cy.contains('Course / Degree').should('exist')
    cy.get('select').select('B.Com 1').should('have.value', 'B.Com 1')
    cy.contains('Total Seats').should('exist')
    cy.get('input[name="seatsNo"]').should('exist')
  })
  ...
  it('when user click on seatMatrix manage link,user should navigate to seat matrix/manage page', () => {
    cy.visit('/admissions')
    cy.get('a[href="/admissions/seatMatrix/manage"]').click()
  ...
  it('Verify back button in seat matrix feature', () => {
    ...
    cy.get('th:contains(Course / Degree)').parents('thead').next('tbody').children('tr').first().find('a:contains(B.Com 1)').click() 
